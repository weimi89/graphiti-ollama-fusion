...
- **Фонова обробка пам'яті** — додавання пам'яті може виконуватися у фоновому режимі, виклик MCP повертається негайно; стан завдань зберігається в SQLite, незавершені завдання автоматично відновлюються після перезапуску
...
- **Веб-інтерфейс керування** — вбудована панель моніторингу, перегляд, пошук, візуалізація графа знань, AI-запитання та відповіді, перегляд спільнот, обслуговування якості, масовий імпорт, налаштування під час виконання
- **Багатомовність (i18n)** — повідомлення відповідей підтримують 33 мови (включно з zh-TW / en / zh-CN / ja / pt-BR / ko / es / de / fr тощо; zh-TW/en/zh-CN/ja написані вручну, решта надається шаром generated); інструменти MCP залежать від `SERVER_LANG`, REST API автоматично узгоджує за HTTP `Accept-Language`
...
│   ├── web_api.py                # REST API веб-інтерфейсу керування (30+ кінцевих точок)
│   ├── ollama_graphiti_client.py  # Клієнт LLM Ollama (розподіл за двома моделями)
│   ├── openai_compat_client.py   # Базовий клас LLM, сумісний з OpenAI (json_object + спрощена схема + захист json)
│   ├── glm_client.py             # Клієнт LLM GLM (Zhipu AI) (успадковує OpenAICompatClient)
│   ├── openrouter_client.py      # Клієнт LLM OpenRouter (успадковує OpenAICompatClient)
│   ├── deepseek_client.py        # Клієнт LLM DeepSeek (успадковує OpenAICompatClient)
...
│   ├── task_store.py             # SQLite-персистенція фонових завдань (TaskStore)
...
│   ├── i18n_generated.py         # Автоматично згенеровані мовні перевизначення (GENERATED_MESSAGE_OVERRIDES)
...
│   └── migrate_embeddings.py     # Міграція моделі Embedding (повторне генерування векторів після зміни моделі)
├── docs/                         # Документація
...
- Обслуговування якості — показники якості пам'яті та інструменти очищення
- Масовий імпорт — одноразовий імпорт кількох фрагментів пам'яті (JSON, до 500 записів за раз)
- Налаштування під час виконання — перегляд поточних активних параметрів і коригування деяких з них без перезапуску
...
| `/api/groups/stats` | GET | Статистика вузлів/фактів/фрагментів по кожній групі |
...
| `/api/nodes/{uuid}/relations` | GET | Отримання вхідних/вихідних ребер вузла |
| `/api/search/nodes` | GET | Векторний пошук вузлів |
| `/api/search/facts` | GET | Векторний пошук фактів |
| `/api/search/episodes` | GET | Пошук фрагментів пам'яті |
...
| `/api/memory/add` | POST | Додавання одного запису пам'яті |
| `/api/memory/add-bulk` | POST | Масове додавання пам'яті |
| `/api/memory/add-triplet` | POST | Додавання трійки |
| `/api/import/episodes` | POST | Масовий імпорт фрагментів пам'яті (JSON, до 500 за раз) |
...
| `/api/timeline` | GET | Перегляд хронології |
| `/api/graph/subgraph` | GET | Отримання підграфа (для візуалізації) |
| `/api/graph/all` | GET | Отримання повного графа (для візуалізації) |
| `/api/ask` | GET | AI-відповіді (на основі пошуку по графу) |
| `/api/analytics/top-nodes` | GET | Вузли з високим ступенем зв'язності/частотою доступу |
| `/api/analytics/quality` | GET | Показники якості графа знань |
| `/api/analytics/stale` | GET | Запит застарілих спогадів |
| `/api/analytics/cleanup` | POST | Очищення застарілих спогадів |
| `/api/config` | GET | Отримання поточних активних параметрів (без API-ключів) |
| `/api/config` | PATCH | Оновлення параметрів під час виконання (діє лише для поточного процесу, скидається після перезапуску) |
...
TASK_DB_PATH=data/tasks.db           # Шлях до SQLite-файлу персистенції фонових завдань
...
```bash
# Виконати всі тести (203 шт., близько 1 секунди)
