...
- **Επεξεργασία μνήμης στο παρασκήνιο** — Η προσθήκη μνήμης μπορεί να εκτελεστεί στο παρασκήνιο, με την κλήση MCP να επιστρέφει άμεσα· η κατάσταση εργασιών διατηρείται σε SQLite, με αυτόματη επαναφορά ημιτελών εργασιών μετά από επανεκκίνηση
...
- **Web διεπαφή διαχείρισης** — Ενσωματωμένος πίνακας ελέγχου, περιήγηση, αναζήτηση, οπτικοποίηση γράφου γνώσης, AI ερωταποκρίσεις, περιήγηση κοινοτήτων, συντήρηση ποιότητας, μαζική εισαγωγή, ρυθμίσεις χρόνου εκτέλεσης
- **Πολυγλωσσική υποστήριξη (i18n)** — Τα μηνύματα απόκρισης υποστηρίζουν 33 γλώσσες (συμπεριλαμβανομένων zh-TW / en / zh-CN / ja / pt-BR / ko / es / de / fr κ.ά.· zh-TW/en/zh-CN/ja χειρόγραφα, τα υπόλοιπα παρέχονται από το επίπεδο generated)· τα εργαλεία MCP βασίζονται στο `SERVER_LANG`, ενώ το REST API διαπραγματεύεται αυτόματα βάσει του HTTP `Accept-Language`
...
│   ├── web_api.py                # REST API της Web διεπαφής διαχείρισης (30+ endpoints)
│   ├── ollama_graphiti_client.py  # Πελάτης LLM Ollama (διαχωρισμός διπλού μοντέλου)
│   ├── openai_compat_client.py   # Βασική κλάση OpenAI-συμβατού LLM (json_object + απλοποιημένο schema + εφεδρική προστασία json)
│   ├── glm_client.py             # Πελάτης LLM GLM (Zhipu AI) (κληρονομεί OpenAICompatClient)
│   ├── openrouter_client.py      # Πελάτης LLM OpenRouter (κληρονομεί OpenAICompatClient)
│   ├── deepseek_client.py        # Πελάτης LLM DeepSeek (κληρονομεί OpenAICompatClient)
...
│   ├── task_store.py             # Διατήρηση εργασιών παρασκηνίου σε SQLite (TaskStore)
...
│   ├── i18n_generated.py         # Αυτόματα δημιουργημένες γλωσσικές επικαλύψεις (GENERATED_MESSAGE_OVERRIDES)
...
│   └── migrate_embeddings.py     # Μετεγκατάσταση μοντέλου Embedding (αναπαραγωγή διανυσμάτων μετά την αλλαγή μοντέλου)
├── docs/                         # Τεκμηρίωση
...
- Συντήρηση ποιότητας — Δείκτες ποιότητας μνήμης και εργαλεία εκκαθάρισης
- Μαζική εισαγωγή — Εισαγωγή πολλαπλών επεισοδίων μνήμης με μία φορά (JSON, έως 500 εγγραφές ανά φορά)
- Ρυθμίσεις χρόνου εκτέλεσης — Προβολή τρεχουσών ρυθμίσεων και δυνατότητα προσαρμογής παραμέτρων χωρίς επανεκκίνηση
...
| `/api/groups/stats` | GET | Στατιστικά κόμβων/γεγονότων/επεισοδίων ανά group |
...
| `/api/nodes/{uuid}/relations` | GET | Λήψη εισερχόμενων/εξερχόμενων σχέσεων κόμβου |
| `/api/search/nodes` | GET | Διανυσματική αναζήτηση κόμβων |
| `/api/search/facts` | GET | Διανυσματική αναζήτηση γεγονότων |
| `/api/search/episodes` | GET | Αναζήτηση επεισοδίων μνήμης |
...
| `/api/memory/add` | POST | Προσθήκη μεμονωμένης μνήμης |
| `/api/memory/add-bulk` | POST | Μαζική προσθήκη μνήμης |
| `/api/memory/add-triplet` | POST | Προσθήκη τριάδας |
| `/api/import/episodes` | POST | Μαζική εισαγωγή επεισοδίων μνήμης (JSON, έως 500 εγγραφές) |
...
| `/api/timeline` | GET | Περιήγηση χρονολογίου |
| `/api/graph/subgraph` | GET | Λήψη υπογράφου (οπτικοποίηση) |
| `/api/graph/all` | GET | Λήψη πλήρους γράφου (οπτικοποίηση) |
| `/api/ask` | GET | AI ερωταποκρίσεις (βάσει ανάκτησης γράφου) |
| `/api/analytics/top-nodes` | GET | Κόμβοι με υψηλή συνδεσιμότητα/πρόσβαση |
| `/api/analytics/quality` | GET | Δείκτες ποιότητας γράφου γνώσης |
| `/api/analytics/stale` | GET | Αναζήτηση παρωχημένων μνημών |
| `/api/analytics/cleanup` | POST | Εκκαθάριση παρωχημένων μνημών |
| `/api/config` | GET | Λήψη τρεχουσών ρυθμίσεων (χωρίς API keys) |
| `/api/config` | PATCH | Ενημέρωση τροποποιήσιμων ρυθμίσεων χρόνου εκτέλεσης (ισχύει μόνο για την τρέχουσα διεργασία, επαναφορά μετά επανεκκίνηση) |
...
TASK_DB_PATH=data/tasks.db           # Διαδρομή διατήρησης εργασιών παρασκηνίου σε SQLite
...
```bash
# Εκτέλεση όλων των δοκιμών (203, περίπου 1 δευτερόλεπτο)
