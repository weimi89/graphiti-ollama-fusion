...
- **پس منظر میں میموری پروسیسنگ** — میموری شامل کرنا پس منظر میں چل سکتا ہے، MCP کال فوری طور پر واپس آ جاتی ہے؛ ٹاسک کی حالت SQLite میں محفوظ ہوتی ہے، دوبارہ شروع ہونے پر نامکمل ٹاسکس خود بخود بحال ہو جاتے ہیں
...
- **Web مینجمنٹ انٹرفیس** — بلٹ ان ڈیش بورڈ، براؤزنگ، تلاش، نالج گراف ویژولائزیشن، AI سوال و جواب، کمیونٹی براؤزنگ، معیار دیکھ بھال، بلک امپورٹ، رن ٹائم سیٹنگز
- **کثیر لسانی (i18n)** — جوابی پیغامات 33 زبانوں کی حمایت کرتے ہیں (بشمول zh-TW / en / zh-CN / ja / pt-BR / ko / es / de / fr وغیرہ؛ zh-TW/en/zh-CN/ja ہاتھ سے لکھے، باقی generated پرت سے فراہم)؛ MCP ٹولز `SERVER_LANG` کے مطابق، REST API HTTP `Accept-Language` کے مطابق خود بخود مذاکرات کرتا ہے
...
│   ├── web_api.py                # Web مینجمنٹ انٹرفیس REST API (30+ اینڈ پوائنٹس)
│   ├── ollama_graphiti_client.py  # Ollama LLM کلائنٹ (دوہرا ماڈل تقسیم)
│   ├── openai_compat_client.py   # OpenAI ہم آہنگ LLM بیس کلاس (json_object + سادہ schema + json حفاظتی تحفظ)
│   ├── glm_client.py             # GLM (Zhipu AI) LLM کلائنٹ (OpenAICompatClient سے وراثت)
│   ├── openrouter_client.py      # OpenRouter LLM کلائنٹ (OpenAICompatClient سے وراثت)
│   ├── deepseek_client.py        # DeepSeek LLM کلائنٹ (OpenAICompatClient سے وراثت)
...
│   ├── task_store.py             # پس منظر ٹاسکس SQLite پرسسٹنس (TaskStore)
...
│   ├── i18n_generated.py         # خود بخود تخلیق کردہ زبانوں کی اوور رائیڈز (GENERATED_MESSAGE_OVERRIDES)
...
│   └── migrate_embeddings.py     # Embedding ماڈل منتقلی
├── docs/                         # دستاویزات
...
- معیار دیکھ بھال — میموری معیار کے اشاریے اور صفائی کے ٹولز
- بلک امپورٹ — ایک ساتھ متعدد میموری ٹکڑے درآمد کریں (JSON، ایک بار میں زیادہ سے زیادہ 500)
- رن ٹائم سیٹنگز — موجودہ مؤثر ترتیب دیکھیں اور دوبارہ شروع کیے بغیر بعض پیرامیٹرز کو ایڈجسٹ کریں
...
| `/api/groups/stats` | GET | ہر group کے نوڈز/حقائق/ٹکڑوں کی شماریات |
...
| `/api/nodes/{uuid}/relations` | GET | نوڈ کے داخلی/خارجی کنارے تعلقات حاصل کریں |
| `/api/search/nodes` | GET | نوڈز کی ویکٹر تلاش |
| `/api/search/facts` | GET | حقائق کی ویکٹر تلاش |
| `/api/search/episodes` | GET | میموری ٹکڑوں کی تلاش |
...
| `/api/memory/add` | POST | واحد میموری شامل کریں |
| `/api/memory/add-bulk` | POST | بلک میموری شامل کریں |
| `/api/memory/add-triplet` | POST | ٹرپلٹ شامل کریں |
| `/api/import/episodes` | POST | میموری ٹکڑے درآمد کریں (JSON، زیادہ سے زیادہ 500 فی بار) |
| `/api/memory/tasks` | GET | پس منظر ٹاسکس کی فہرست (حالت فلٹرنگ کی حمایت) |
| `/api/memory/tasks/{id}` | GET | واحد ٹاسک کی حالت کی تلاش |
| `/api/timeline` | GET | ٹائم لائن براؤزنگ |
| `/api/graph/subgraph` | GET | ذیلی گراف حاصل کریں (ویژولائزیشن) |
| `/api/graph/all` | GET | مکمل گراف حاصل کریں (ویژولائزیشن) |
| `/api/ask` | GET | AI سوال و جواب (گراف پر مبنی بازیافت) |
| `/api/analytics/top-nodes` | GET | اعلیٰ ربط/اعلیٰ رسائی نوڈز |
| `/api/analytics/quality` | GET | نالج گراف معیار کے اشاریے |
| `/api/analytics/stale` | GET | پرانی میموریز کی تلاش |
| `/api/analytics/cleanup` | POST | پرانی میموریز کی صفائی |
| `/api/config` | GET | موجودہ مؤثر ترتیب حاصل کریں (API key کے بغیر) |
| `/api/config` | PATCH | رن ٹائم میں قابلِ ترمیم ترتیب کو اپ ڈیٹ کریں (صرف اس پروسیس کے لیے، دوبارہ شروع پر واپس) |
...
TASK_DB_PATH=data/tasks.db           # پس منظر ٹاسکس SQLite پرسسٹنس پاتھ
...
```bash
# تمام ٹیسٹس چلائیں (203، تقریباً 1 سیکنڈ)
