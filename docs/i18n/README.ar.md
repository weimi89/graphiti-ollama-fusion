...
- **معالجة الذاكرة في الخلفية** — يمكن تنفيذ إضافة الذاكرة في الخلفية، مع عودة فورية لاستدعاء MCP
...
- **واجهة الإدارة عبر الويب** — تتضمن لوحة معلومات وتصفّحًا وبحثًا وتصوّرًا للرسم البياني المعرفي وأسئلة وأجوبة بالذكاء الاصطناعي وتصفّح المجتمعات
- **تعدّد اللغات (i18n)** — تدعم رسائل الاستجابة أكثر من 30 لغة محلية (تشمل zh-TW / en / zh-CN / ja / pt-BR / ko / es / de / fr وغيرها)؛ تعتمد أدوات MCP على `SERVER_LANG`، بينما تتفاوض REST API تلقائيًا اعتمادًا على ترويسة HTTP `Accept-Language`
...
│   ├── web_api.py                # REST API لواجهة الإدارة عبر الويب (20+ نقطة نهاية)
│   ├── ollama_graphiti_client.py  # عميل LLM لـ Ollama (توزيع نموذجين)
│   ├── glm_client.py             # عميل LLM لـ GLM (الذكاء الاصطناعي من Zhipu) (API متوافق مع OpenAI)
│   ├── openrouter_client.py      # عميل LLM لـ OpenRouter (تجميع نماذج من مزوّدين متعددين)
│   ├── deepseek_client.py        # عميل LLM لـ DeepSeek (json_object + حماية json احتياطية)
...
```bash
# تنفيذ جميع الاختبارات (183 اختبارًا، حوالي ثانية واحدة)
