...
- **পটভূমি মেমরি প্রক্রিয়াকরণ** — মেমরি যোগ পটভূমিতে চলতে পারে, MCP কল অবিলম্বে ফিরে আসে; টাস্ক স্ট্যাটাস SQLite-এ স্থায়ীভাবে সংরক্ষিত হয়, পুনরায় শুরু করলে অসম্পন্ন টাস্ক স্বয়ংক্রিয়ভাবে পুনরুদ্ধার হয়
...
- **Web ব্যবস্থাপনা ইন্টারফেস** — অন্তর্নির্মিত ড্যাশবোর্ড, ব্রাউজিং, অনুসন্ধান, জ্ঞান গ্রাফ ভিজুয়ালাইজেশন, AI প্রশ্নোত্তর, সম্প্রদায় ব্রাউজিং, গুণমান রক্ষণাবেক্ষণ, ব্যাচ আমদানি, রানটাইম সেটিং
- **বহুভাষিক (i18n)** — প্রতিক্রিয়া বার্তা ৩৩টি ভাষা সমর্থন করে (zh-TW / en / zh-CN / ja / pt-BR / ko / es / de / fr ইত্যাদি সহ; zh-TW/en/zh-CN/ja হাতে লেখা, বাকিগুলো generated স্তর দ্বারা প্রদত্ত); MCP টুল `SERVER_LANG` অনুসারে, REST API HTTP `Accept-Language` অনুসারে স্বয়ংক্রিয়ভাবে সমঝোতা করে
...
│   ├── web_api.py                # Web ব্যবস্থাপনা ইন্টারফেস REST API (30+ এন্ডপয়েন্ট)
│   ├── ollama_graphiti_client.py  # Ollama LLM ক্লায়েন্ট (দ্বৈত মডেল বিভাজন)
│   ├── openai_compat_client.py   # OpenAI সামঞ্জস্যপূর্ণ LLM বেস ক্লাস (json_object + সরলীকৃত schema + json সুরক্ষা)
│   ├── glm_client.py             # GLM (Zhipu AI) LLM ক্লায়েন্ট (OpenAICompatClient থেকে উত্তরাধিকারী)
│   ├── openrouter_client.py      # OpenRouter LLM ক্লায়েন্ট (OpenAICompatClient থেকে উত্তরাধিকারী)
│   ├── deepseek_client.py        # DeepSeek LLM ক্লায়েন্ট (OpenAICompatClient থেকে উত্তরাধিকারী)
...
│   ├── task_store.py             # পটভূমি টাস্ক SQLite স্থায়ীকরণ (TaskStore)
...
│   ├── i18n_generated.py         # স্বয়ংক্রিয়ভাবে তৈরি ভাষা ওভাররাইড (GENERATED_MESSAGE_OVERRIDES)
...
│   └── migrate_embeddings.py     # Embedding মডেল মাইগ্রেশন
├── docs/                         # ডকুমেন্টেশন
...
- গুণমান রক্ষণাবেক্ষণ — মেমরি গুণমান সূচক ও পরিষ্কার টুল
- ব্যাচ আমদানি — একবারে একাধিক মেমরি খণ্ড আমদানি (JSON, একবারে সর্বোচ্চ 500টি)
- রানটাইম সেটিং — বর্তমান কার্যকর সেটিং দেখুন, পুনরায় শুরু না করেই কিছু প্যারামিটার পরিবর্তন করুন
...
| `/api/memory/add` | POST | একক মেমরি যোগ |
| `/api/memory/add-bulk` | POST | ব্যাচ মেমরি যোগ |
| `/api/memory/add-triplet` | POST | ট্রিপলেট যোগ |
| `/api/import/episodes` | POST | মেমরি খণ্ড ব্যাচ আমদানি (JSON, একবারে সর্বোচ্চ 500টি) |
...
| `/api/timeline` | GET | টাইমলাইন ব্রাউজিং |
| `/api/graph/subgraph` | GET | সাব-গ্রাফ পান (ভিজুয়ালাইজেশন) |
| `/api/graph/all` | GET | সম্পূর্ণ গ্রাফ পান (ভিজুয়ালাইজেশন) |
| `/api/ask` | GET | AI প্রশ্নোত্তর (গ্রাফ-ভিত্তিক পুনরুদ্ধার) |
| `/api/analytics/top-nodes` | GET | উচ্চ-সংযুক্ততা/উচ্চ-অ্যাক্সেস নোড |
| `/api/analytics/quality` | GET | জ্ঞান গ্রাফ গুণমান সূচক |
| `/api/analytics/stale` | GET | পুরনো মেমরি অনুসন্ধান |
| `/api/analytics/cleanup` | POST | পুরনো মেমরি পরিষ্কার |
| `/api/config` | GET | বর্তমান কার্যকর সেটিং পান (API key ছাড়া) |
| `/api/config` | PATCH | রানটাইমে পরিবর্তনযোগ্য সেটিং আপডেট (শুধু বর্তমান প্রসেসে কার্যকর, পুনরায় শুরুতে পূর্বাবস্থায় ফিরে) |
...
TASK_DB_PATH=data/tasks.db           # পটভূমি টাস্ক SQLite স্থায়ীকরণের পথ
...
```bash
# সমস্ত টেস্ট চালান (203টি, প্রায় 1 সেকেন্ড)
